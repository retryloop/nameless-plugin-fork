...
public abstract class AbstractLogger implements Reloadable {
	...
	private boolean printReducedException(Throwable t, String lastErrorCommand) {
		...
		if (t instanceof com.namelessmc.java_api.exception.NamelessException && shouldPrintNamelessMessage(t)) {
			this.warning(t.getMessage() + " " + lastErrorCommand);
			return true;
		}
	...
	private static boolean shouldPrintNamelessMessage(Throwable t) {
		final String message = t.getMessage();
		if (message == null || message.isBlank()) {
			return false;
		}

		return message.startsWith("Website returned invalid response with code ")
				|| message.startsWith("Network connection error (not a Nameless issue). ")
				|| message.startsWith("Website returned empty response with status code ")
				|| message.startsWith("Website returned a redirect. ");
	}
