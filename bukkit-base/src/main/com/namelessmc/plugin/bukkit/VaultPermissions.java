...
public class VaultPermissions extends AbstractPermissions {
	...
	@Override
	public CompletableFuture<Set<String>> getPlayerGroups(final @NonNull UUID uuid) {
		if (this.permission == null) {
			throw new ProviderNotUsableException();
		}
		return CompletableFuture.completedFuture(Arrays.stream(
				this.permission.getPlayerGroups((String) null, Bukkit.getOfflinePlayer(uuid)))
